...
def generate_and_send_otp(user):
    ...
    try:
        ...
        print(f"Tentative d'envoi d'email à {user.email}") #debug a enlever
        print(f"From: {from_email}") # same
    ...
    if request.method == "GET":
        ...
        if not otp_code:
            try:
                ...
                print(f"Erreur lors de la génération/envoi OTP: {str(e)}")
                return JsonResponse({
                    "success": False,
                    "error": "Erreur technique lors de l'envoi du code"
                }, status=500)
        ...
        try:
            ...
            print(f"Erreur lors de la vérification OTP: {str(e)}")
            return JsonResponse({
                "success": False,
                "error": "Erreur technique lors de la vérification du code"
            }, status=500)
